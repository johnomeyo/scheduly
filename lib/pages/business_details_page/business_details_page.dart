...
class BusinessDetailsPage extends StatelessWidget {
  ...
  void _showBookingSheet(BuildContext context, ServiceModel service) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Theme.of(context).colorScheme.surface, // Use theme directly
      shape: const RoundedRectangleBorder(
        borderRadius: BorderRadius.vertical(top: Radius.circular(24)),
      ),
      builder: (context) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.transparent,
        elevation: 0,
        iconTheme: IconThemeData(color: _applyOpacity(Colors.white, 1.0)), // Use white explicitly
        actions: [
          IconButton(icon: const Icon(Icons.share), onPressed: () {
      ...
      body: CustomScrollView(
        slivers: [
          ...
          SliverToBoxAdapter(
            child: Transform.translate(
              offset: const Offset(0, -20), // Adjust offset if needed after header refactor
              child: Container(
                padding: const EdgeInsets.symmetric(
                  horizontal: 24,
                  vertical: 16,
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        OutlinedButton.icon(
                          ...
                          onPressed: () {
                            // TODO: Implement contact functionality (e.g., call, open contact modal)
                          },
                          icon: const Icon(Icons.phone),
                          label: const Text('Contact'),
                        ),
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 16),
                        // Feature Chips
                        Wrap( // Use Wrap for better responsiveness with chips
                           spacing: 8, // Horizontal spacing between chips
                           runSpacing: 8, // Vertical spacing between rows of chips
                           children: business.features.map(
                             (featureText) {
                                // Assuming featureText can be mapped to icons
                                IconData iconData = Icons.check_circle; // Default icon
                                // TODO: Add logic to select icon based on featureText if needed
                                if (featureText.toLowerCase().contains('open')) {
                                  iconData = Icons.schedule;
                                } else if (featureText.toLowerCase().contains('time')) {
                                   iconData = Icons.watch_later;
                                } // Add more conditions as needed

                                return FeatureChip( // Use Extracted FeatureChip widget
                                  icon: iconData,
                                  label: featureText,
                                );
                              },
                  ...
                  ...business.services.map(
                    (service) => ServiceCard(
                      ...
                      onBookPressed: () => _showBookingSheet(context, service), // Pass callback
                    ),
                  ),
                  const SizedBox(height: 24),
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      TextButton(
                        onPressed: () {
                          // TODO: Navigate to Write Review Page or show modal
                        },
                        child: const Text('Write a Review'),
                      ),
                    ],
